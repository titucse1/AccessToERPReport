...
class Package_list extends CI_Controller
{
	...
	function index()
	{
		$data['PackageList']=$this->Package_listModel->getPackage_list();
	...
	function selectPackage_list($Product_Package_Id='')
	{	
		$PackageList=$this->Package_listModel->getPackage_list($Product_Package_Id);
	  ...
	  <div class="table-responsive">
       <div class="buttonDiv" style="text-align: center;">
            <button class="btn btn-success" onClick="selectSinglePackage_list()" type="button"><span class="icon-ok"></span>&nbsp;Select Package</button>
       </div>
	...
	function submit()
	{
		 ...
		 if($this->input->post('submit'))
		 {
			 ...
			 $inserted = FALSE;
			 $uniquePackage=$this->Package_listModel->uniquePackage($Product_Package_Name,$Product_Package_Id);
			 
			 if($Product_Package_Name!='' && $uniquePackage==TRUE)
			 $inserted=$this->Package_listModel->savePackage($Product_Package_Id,$insertArray); //savePackage_list() is used for  Insert and Update IDCR_TBL_PRODUCT_PACKAGE.
	...
	function edit_package($Product_Package_Id='')
	{
		...
		$Package=$this->Package_listModel->getPackage_listSingle($Product_Package_Id);
		echo '
			  ...
			  <div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel"><span class="icon-edit"></span>Edit Package</h4>
						...
						<div class="modal-body">
							<form id="validation2" class="bs-example bs-example-form" method="post" enctype="multipart/form-data" action="'.base_url().'package_list/submit/">
